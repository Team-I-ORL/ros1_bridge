...
RUN if [[ "$ADD_ros_tutorials" = "1" ]]; then                                           \
     ...
     git clone https://github.com/Team-I-ORL/ros1_bridge.git &&                                   \
     cd ros1_bridge/ &&                                                                 \
     git checkout orb_robtics &&                                                            \
     cd ../.. &&                                                                        \
...
# Define a mount point for external volumes, provide a default path
VOLUME ["/docker_dir"]

ENTRYPOINT ["/bin/bash"]
#CMD ["source /opt/ros/humble/setup.bash", "source ros-humble-ros1-bridge/install/local_setup.bash"]
